...
int fromJDaysToMonth(int jDay, int year)
{
	int Days=0;
...
int CalcDateFromJulianDay(int aJulianday, int year)
{
	int monthday[12]={31,28,31,30,31,30,31,31,30,31,30,31};
	if(IsLeapYear(year))
		monthday[1]=29;
	int month =fromJDaysToMonth(aJulianday, year)-1;
	int days=0;;
	for(int i=0;i<month;i++)
	{
		days+=monthday[i];
	}
	days=aJulianday-days;
   return (days+1);
}
...
/**
startDay,stopDay: Julian days, all between first and second number incl.
startYear,stopYear:all between first and second number incl. - max possible 2000

ncuStart, ncuStop:all between first and second number inclusive - max possible 40991
yearstart: first year considered in the climate data set
yearsfile: number of years stored in one set of files
clim_inpath[200]:input folder
clim_outpath[200]: output folder
Cclim_name1[200]: trunc of file name for climate data
 */
void function(int startDay, int stopDay, int startYear, int stopYear, int ncuStart,int ncuStop,int yearstart, int yearsfile, char clim_inpath[200], char clim_outpath[200], char Cclim_name1[200])
{
	int read_start[maxpars], read_end[maxpars];
	read_start[0]=ncuStart;
	read_end[0]=ncuStop;
	...
	//! clim_map1 is map file for the binary file clim_bin1. clim_bin1 is the binary data file.  clim_map and clim_bin2 are not used for REMO data
	    FILE *clim_map1, *clim_bin1, *clim_bin2;
							clim_bin1 = NULL;
					clim_bin2 = NULL;
		...
		char Cclim_map1
			[200], Cclim_map2[200], Cclim_bin1[200];

		 strcpy(Cclim_map1, Cclim_name1);
	    ...
	    long int posmap1,posmap2,posbin1,posclim,ldummy1;
	 ...
	 /*   sprintf(Cclim_map1,"meteodataextraceannual.txt");
	    ...
	    int startbyte,numdim,dim1_gap=1,startpix,nnendpix;
	    ...
	    if(clim_type=="remo"){
	    	...
	    	cout<<Cclim_map1<<endl;
	    }
		if (clim_type == "NSU"){
			clim_bin2 = fopen(Cclim_map1, "rb");
			}
	    ...
	    if(clim_type=="remo" || clim_type=="ncushort"){
	    ...
	    for (int para=read_start[3];para<=read_end[3];para++)
	    {
	     ...
	     sprintf(Cclim_writeday,"%s/climatData.clm",clim_outpath);
	 //   sprintf(Cclim_writeday,"%s/%s%d-%d.%d-%d.%d-%d.%d-%d_dy.cml",clim_outpath,clim_type.c_str(),read_start[0],read_end[0],read_start[1],read_end[1],read_start[2],read_end[2],read_start[3],read_end[3]);
	    clim_writeday=fopen(Cclim_writeday,"w");  //rb

	    if (clim_writeday==NULL){cout << "Cannot open clim_writeday " << Cclim_writeday <<"\n"; exit(0);}

	  /*  if(writing>2)fprintf(clim_writeday,"year,month,jday");
	    for (int para=read_start[3];para<=read_end[3];para++)
	    {
	        if(writing>2)fprintf(clim_writeday,",%s",pars[para]);
	    }
	    fprintf(clim_writeday,",epoc");*/
	        ...
	        for (int ncu=read_start[0];ncu<=read_end[0];ncu+=dim_stp[0])
	        {
	            ...
	            else
	            {
	                ...
	                for (int day=read_start[2];day<=read_end[2];day+=dim_stp[2])
	                {
	                    ...
	                    if(writing>2)fprintf(clim_writeday,"%d %d %d",year,fromJDaysToMonth(day,year),CalcDateFromJulianDay(day,year));
	                    //!
	                    float pre=0;
	                    float meanTemp;
	                    float rat;
	                    for (int para=read_start[3];para<=read_end[3];para++)
	                    {
	                       ...
	                       // cout<<para<<" "<<fdummy<<" "<<pars[para]<<endl;
	                        if(writing>2)
	                        {
	                        	if(para==0)
	                        	{
	                        	...
	                        	{
	                        		rat=fdummy;
	                        		fprintf(clim_writeday," %f",fdummy);
	                        		fprintf(clim_writeday," %f",pre);
	                        	}
	                        	else
	                        		fprintf(clim_writeday," %f",fdummy);
	                        	if(para==3)
	                        	{
	                        		double tmp=MakkinkEvaporation(rat,meanTemp);

	                        		fprintf(clim_writeday," %f \n",tmp);
	                        	}
	                        }
...
int getYearFilestartOld(int start)
{
	if(start>2000)
	{
		cout<<"illigal year"<<endl;
		exit(99);
	}
	else if(start>1990)
		return 1991;
...
int getYearFilestartNew(int start)
{
	if(start>2030)
	{
		cout<<"illigal year"<<endl;
...
void function(int startDay, int stopDay, int startYear, int stopYear, int ncuStart,int ncuStop, char newPath[200],char oldPath[200], char clim_outpath[200], char Cclim_nameNew[200], char Cclim_nameOld[200])
{

	cout<<newPath<<endl;
	cout<<oldPath<<endl;
	if(startYear<2001&&stopYear<2001)
	{

		function(startDay, stopDay, startYear, stopYear, ncuStart,ncuStop,getYearFilestartOld(startYear),  10, oldPath, clim_outpath,  Cclim_nameOld);
	}
	else if(startYear<2001&&stopYear>=2001)
	{
		cout<<"done1"<<endl;
		function(startDay, stopDay, startYear, 2000, ncuStart,ncuStop,getYearFilestartOld(startYear),  10, oldPath, clim_outpath,  Cclim_nameOld);
		cout<<"done2"<<endl;
		function(startDay, stopDay, 2001, stopYear, ncuStart,ncuStop,getYearFilestartNew(2001),  10, newPath, clim_outpath,  Cclim_nameNew);
		cout<<"done3"<<endl;
	}
	else if (startYear>=2001&&stopYear>=2001)
	{
		function(startDay, stopDay, startYear, stopYear, ncuStart,ncuStop,getYearFilestartNew(startYear),  10, newPath, clim_outpath,  Cclim_nameNew);
	}
	cout<<"done"<<endl;
}
int main()
{
	...
	char oldPath[200];
	char newPath[200];
	char clim_outpath[200];
	char Cclim_nameNew[200];
	char Cclim_nameOld[200];
	string output="c:\\trash";
	string oldPaths="k:\\NJH\\ds199\\ncushort";
	string newPaths="k:\\NJH\\a1b";
	string baseFileNameNew="remoA1B_ncu";
	string baseFileNameOld="marsateam_ncu";
	strcpy(oldPath,oldPaths.c_str());
	strcpy(newPath,newPaths.c_str());
	strcpy(clim_outpath,output.c_str());
	strcpy(Cclim_nameNew,baseFileNameNew.c_str());
	strcpy(Cclim_nameOld,baseFileNameOld.c_str());
	function(1,365,1979,1999,1,40,newPath,oldPath,clim_outpath,Cclim_nameNew, Cclim_nameOld);
 }   
