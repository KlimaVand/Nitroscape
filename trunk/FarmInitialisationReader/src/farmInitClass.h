...
/*	struct OpList {
		vector <fieldOp> theOps;
	} ;
	struct crops {
		vector<OpList> cropList;
	} ;

	crops theListOfCrops;*/

/*	linkList<fieldOp> *theCropOps;
	linkList<linkList<fieldOp>> *theCropOpsList;*/
...
public:
	...
	/*!
	 * @param appendit if true, the output will be appended to the outfile
	 */
	bool writeFieldOps(bool appendit);
	...
	*/
	...
	void FindCropOps(string cropName, int opNum, fstream *outfile);
};
