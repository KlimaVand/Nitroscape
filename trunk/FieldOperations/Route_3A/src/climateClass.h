/****************************************************************************\
Class "climateClass"
Handles climateClass data, provided by an ascii-file.
...
class climateClass: public base
{
	enum {strlength=255};
   ...
   bsTime FirstDate;            // First date in climateClass database
	bsTime LastDate;             // Last date in climateClass database
   ...
   bool Circular;                      // If using circular dates!
	...
	climateClass& operator=(const climateClass& c); // Dissable the compilers generation of default assignment operator.
	climateClass(const climateClass& c);            // Dissable the compilers generation of default copy constructor.
...
public:
   ...
   climateClass(){filetyp = 0;EpotCalculationMethod = 0; found = false;};
   climateClass(int aType) {filetyp = aType; EpotCalculationMethod = aType;};
   climateClass(int aType,int aMethod) {filetyp = aType; EpotCalculationMethod = aMethod;};
   ~climateClass();
   ...
   void InitClimate(char *aDirectory,char *filename, int climatefileType);                                      // automaticly recognise the type
	void Update();
   ...
   double Getprecip(){return precip;};
   double GetEPot(){return epot;};
};

//extern climateClass * theClimate;
