...
//			f1 >> date >> tmin >> tmax >> tmean >> precip >> rad  >> humidity >> vel;
	      ...
	      windVelocity = GetWindAtHeight(10.0,vel,2.0,0.01);                 // calculates the velocity at 2m (vel is at 10m)
			file->getline(buffer,strlength);
         break;
      case 3:	// DAISY climateClass files
         char c[20];
         file->get(c,3);
         yr = atoi(c)+1900;
         file->get(c,3);
         ...
         file->getline(buffer,strlength);
         break;
      case 4:	// DAISY climateClass files but with year 2000 compatibility and extra column with windspeed
         char buf[20];
         file->get(buf,5);
         yr = atoi(buf);
         file->get(buf,3);
...
\****************************************************************************/
void climateClass::UpdateMeasuredSoilTemperature()
{
...
/****************************************************************************\
\****************************************************************************/
double climateClass::GetNextSoilTemperature()
{
	double temperature;
...
/****************************************************************************\
Initializes the climateClass object.
Can be used an arbitrary number of times in one simulation if needed.
Parameters:
   filename   name of ascii-file with climateClass data
   filetype   probably temporarely
   firstobs   states the number of headerlines without data in climateClass file
\****************************************************************************/
void climateClass::InitClimate(char * filename, int filetype, int firstobs)
{
   Circular = false;
...
Parameters:
   ...
   firstobs   states the number of headerlines without data in climateClass file
\****************************************************************************/
void climateClass::InitClimate(char *aDirectory, char * filename, int climatefileType)
{
  filetyp=	climatefileType;
  double date;
  ...
  if (!FileExists(filename))
   	theMessage->FatalError("climateClass file named [",filename,"] not found");
...
/****************************************************************************\
\****************************************************************************/
double climateClass::GetCO2Concentration()
{
   if (CO2Concentration > 2000 || CO2Concentration < 100)
      theMessage->FatalError("climateClass::GetCO2Concentration - CO2 concentration beyond reasonable borders");
   return CO2Concentration;
}

/****************************************************************************\
\****************************************************************************/
double climateClass::VaporPress(double temp)
{
	double ret_val;
   if (temp<0)
...
/****************************************************************************\
\****************************************************************************/
double climateClass::DeltaVaporPress(double temp)
{
	double ret_val;
...
/****************************************************************************\
\****************************************************************************/
double climateClass::SunDeclination()
{
...
\****************************************************************************/
double climateClass::PhotoPeriod()
{
...
\****************************************************************************/
double climateClass::PenmanEvaporation(double globalRadiation,double temp,double windSpeed,double humidity)
{
...
af potentiel fordampning. Statens Planteavlsfors�g. Beretning nr. S 2157. 1991
\****************************************************************************/
double climateClass::MakkinkEvaporation(double globalRadiation,double temp)
{
   const double evpHtWat=2.465;
   const double psykrConst=0.667;
...
\****************************************************************************/
double climateClass::Evaporation(double radiation,double temp,double humidity, double transferCoefficient)
{
...
\****************************************************************************/
double climateClass::Evaporation(double temp,double humidity, double transferCoefficient)
{
...
\****************************************************************************/
double climateClass::AdjustPrecip(int month)
{
	return CorrectionFactor[month];
}

/****************************************************************************\
Obtains climateClass data for a specific date
Parameters:
   year, month, day
...
\****************************************************************************/
void climateClass::GetClimate(int year, int month, int day, double &temp, double &min_temp,
...
/****************************************************************************\
\****************************************************************************/
void climateClass::GetClimate(double &temp, double &min_temp,
						 ...
						 double &EPot, double &windspeed, double &relHumidity)
{
  ...
  else
  {
   ...
   relHumidity = 0.0;
   windspeed = 0.0;
   theMessage->WarningWithDisplay("climateClass::GetClimate - today's weather has not been found yet");
   cout << "";
  }
}
...
Obtains climateClass data for the current day
...
void climateClass::Update()
{
   ...
   if (theTime>LastDate)
   {
   	if (!Circular) theMessage->WarningWithDisplay("climateClass::Current date greather than last date - using circular dates!");
      Circular = true;
   ...
   if (!found && Circular && month==2 && day>=29)
   	found = true;                 // a fix to ensure correct circular referencing
   if (!found)
   	theMessage->Warning("climateClass::DailyUpdate - date not in climateClass-database: ");
...
Parameters:
   year, month, day
\****************************************************************************/
bool climateClass::FindClimate(int year, int month, int day)
{
   char buffer[MAX_TXT];
   getcwd(buffer, MAX_TXT); //get current directory - this procedure will divert to climateClass directory
   bool circular = false;
   found = false;
   if (year<firstYear)
   	year = firstYear;
...
/****************************************************************************\
Update climateClass to current day
Author: JBE
  ...
  // 	precip = precip*AdjustPrecip(month);
   ...
   if (co2>100)
   {
      if (co2<2000)
         ...
         theMessage->FatalError("climateClass::Update - CO2 concentration at an unrealistic high level");
   }
   ...
   if (!found && Circular && month==2 && day>=29)
   	found = true;                 // a fix to ensure correct circular referencing
   if (!found)
   	theMessage->WarningWithDisplay("climateClass::DailyUpdate - date not in climateClass-database: ");
...
Presently it does not !!!
\****************************************************************************/
void climateClass::Update(bsTime *aTime)
{
   char buffer[MAX_TXT];
   getcwd(buffer, MAX_TXT); //get current directory - this procedure will divert to climateClass directory
...
/****************************************************************************\
\****************************************************************************/
void climateClass::CalcAverageClimate()
{
   char buffer[MAX_TXT];
   getcwd(buffer, MAX_TXT); //get current directory - this procedure will divert to climateClass directory
   for (int i=0; i<13; i++)
   ...
   if (numYears==0)
	   theMessage->FatalError("climateClass::Less than one year of data in climateClass database");
   if (numYears<5)
   	theMessage->Warning("climateClass::average climateClass based on <5 years of data");
...
//  theClimate->Update();  //reset climateClass to current date
  Update();  //reset climateClass to current date
  chdir(buffer);   //revert to previous directory
}

/****************************************************************************\
\****************************************************************************/
void climateClass::CalcAverageClimateForPeriod(bsTime *startDate, int numOfDays, double *aveTemperature, double *aveWindspeed)
{
   char buffer[MAX_TXT];
   getcwd(buffer, MAX_TXT); //get current directory - this procedure will divert to climateClass directory
   int count=0;
...
//   theClimate->Update();  //reset climate to current date
   Update();  //reset climate to current date
   chdir(buffer);   //revert to previous directory
};
