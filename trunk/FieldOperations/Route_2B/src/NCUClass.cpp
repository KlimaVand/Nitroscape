...
//		strcpy(inputFileName,"WinWheat.txt");
		...
		for (int cropNo = 0; cropNo<acropRotationClass->gettheCrops()->NumOfNodes(); cropNo++)
		{
			...
			if (aNCUCropClass)
			{
				...
				{
					if (routeNumber==1)
						inputFileName="SpWheat.csv";
					else
						inputFileName="SpWheat.txt";
					diff = 126;
					defaultsowingDay = 99;
				...
				{
					if (routeNumber==1)
						inputFileName="Potato.csv";
					else
						inputFileName="Potato.txt";
					diff = 137;
				...
				{
					if (routeNumber==1)
						inputFileName="FodMaize.csv";
					else
						inputFileName="FodMaize.txt";
					diff = 112;
				...
				{
					if (routeNumber==1)
						inputFileName="GraMaize.csv";
					else
						inputFileName="GraMaize.txt";
					diff = 165;
				...
				{
					if (routeNumber==1)
						inputFileName="WinRape.csv";
					else
						inputFileName="WinRape.txt";
					diff = 306;
				...
				{
					if (routeNumber==1)
						inputFileName="SugBeet.csv";
					else
						inputFileName="SugBeet.txt";
					diff = 169;
				...
				{
					if (routeNumber==1)
						inputFileName="SunFlower.csv";
					else
						inputFileName="SunFlower.txt";
					diff = 150;
				...
				if (localName->compare("SETA")!=0)   //skip harvesting for set-aside
				{
					...
					if ((localName->compare("GRAS")==0)||(localName->compare("GRAP")==0))
					{
						...
						{
							earliestStart=sowingDay + 60; //spare the permanent grass from grazing in the start of the first year
						}
						//default dates for start and stop of grazing dairy cattle
						int grazingDays;
						if ((springGrazing)&& (year>=implementationYear)) //implement spring grazing
						{
							//dairy cattle
						...
						else	//no spring grazing or is before implementation year
						{
							...
							if (grazingDays>0)
							{
								double desiredStartDay = 180.0-grazingDays/2;
								if (earliestStart >= round(desiredStartDay))
									aNCUCropClass->setGrazingStartDay(earliestStart,0,0);
								else
						...
						if (earliestStart>=springBarleySowing)// need to reduce number of cuts
							lengthOfGrowingSeason -=(earliestStart-springBarleySowing);
...
//!Check to make sure that the event requested is either Sowing or harvesting
   ...
   {
#ifdef VS2008
	for(int j=0;j<MARSDateFileName.length();j++)
   ...
   if(found==false)	//this crop has not been read into a vector yet, so get it
   {
	   File file;
#ifdef VS2008
	for(int i=0;i<MARSDateFileName.length();i++)
...
void NCUClass::checkSlurryOperations(cropSeriesClass * acropSeriesClass, NCUCropClass *aNCUCropClass, int calendarYear)
{
