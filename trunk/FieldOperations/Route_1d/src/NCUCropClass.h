...
class NCUCropClass
{
     ...
     int PloughingDay[5];
     int HarrowingDay[5];
     //!Manure application dates; 0= Cattle solid, 1=cattle slurry, 2=pig solid, 3=pig slurry, 4=poultry solid, 5=poultry slurry, 6=grazing
     int ManureDay[numberManureTypes];
     //!Fertiliser application dates; 0= urea, 1=nitrate
     int FertDay[numberFertTypes];
...
public:
  ...
  string getCropName() const  { return CropName;}
   ...
   int getManureDay(int index) {return ManureDay[index];}
   int getFertDay(int index) {return FertDay[index];}
  ...
  void setCropName (string aCropName)  { CropName = aCropName;}
   ...
   void setWinterCrop(bool aVal){WinterCrop = aVal;}
	...
	void setManureDay(int aVal, int index){ManureDay[index]=aVal;}
	void setFertDay(int aVal, int index){FertDay[index]=aVal;}
...
#else
	  bool ReadData(stringstream *instring, int row);
#endif
