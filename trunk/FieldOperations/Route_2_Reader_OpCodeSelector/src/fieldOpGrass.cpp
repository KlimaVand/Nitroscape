...
void fieldOpGrass::DefineOp(char * aName, int aNumber, int Year, int Month, int Day, int theGrazingType, double LAI)
{
...
void fieldOpGrass::DefineOp(char * aName, int aNumber, int Year, int JulianDay, int theGrazingType, double LAI)
{
...
void fieldOpGrass::outputOps(fstream * outfile, int status, bool printYear)
{
	...
	if (opCode==5)
	{
		*outfile << grazingType << "\t" << harvLAI << "\t";
		if (verbosePrint)
			cout << grazingType << "\t" << harvLAI << "\t";
		numDumint=4;
	}
	else
	{
