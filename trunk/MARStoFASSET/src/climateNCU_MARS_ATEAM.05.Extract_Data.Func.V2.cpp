...
int getMARS(int hsmuID,char Cmars[100]);
...
int getNCU_ID(int hsmu,char Cncu[100])
{
    ...
    if (ncu==NULL)
    {
        ...
        cout << buffer << endl;
    	theMessage->FatalError("getNCU_ID: file ", Cncu, " not found");
    }
    ...
    fclose(ncu);
    if (hsmuID==999)
    	theMessage->FatalError("getNCU_ID: NCU ID for hsmu ", hsmu, " not found");
    return hsmuID;
}
int getNCUfromHSMU(int hsmuID,char Cncu[100])
{
    FILE *ncu;
    //char Cncu[100];
    ncu=fopen(Cncu, "r");
    ...
    fclose(ncu);
    if (hsmuID==999)
    	theMessage->FatalError("getNCUfromHSMU: NCU for ", hsmuID, " not found");
    return hsmu;
}
...
int getMARS(int hsmuID,char Cmars[100])
{
    ...
    fclose(mars);
    if (marsID==0){theMessage->FatalError("getMARS - no MARS code for this HMSU");}
    return marsID;
}
