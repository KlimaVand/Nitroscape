...
//
 ...
 //following two lines are only needed if simulating ecosystem here
   ...
   string PixiDataPath;
   aBase.GetParameter("PixiDataPath",&PixiDataPath);
   ...
   aBase.SetCritical();
    ...
    aBase.UnsetCritical();
   int startFarm =0;  //first farm to simulate
   int stopFarm =0; //last farm to simulate
   if (aBase.GetParameter("JumpStart",&startFarm))
      {
   	   aBase.GetParameter("JumpStop",&stopFarm);
