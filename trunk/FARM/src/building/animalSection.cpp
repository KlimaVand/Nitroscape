 ...
 */
void animalSection::ReadParameters(fstream *file, bool defaultValus)
{
	Setfile(file);
	Setcur_pos(0);
	UnsetCritical();
   ...
   // ensure cumulative excreta partitioned to flooring = 1
    if (cumulativePartition!=1.0)
			...
			flooring->ElementAtNumber(0)->ReadParameters(file);
	}
   Setfile(NULL);
}
