...
//				FindSection(InstanceName,inx);
   ...
   // Dungheap not tested
      ...
      GetSectionNumbers(InstanceName,&first,&num);
         num=0;
			if (num>0)
      {
				defaultdungheap = new dungheap(InstanceName,0,this);
				defaultdungheap->ReadDefaultParameters();
			}
			Setcur_pos(0);
			for(int inx=first;inx<(first+num);inx++)
			{
				SetCritical();
				FindSection(InstanceName,inx);
				dungheap *manureInst=new dungheap(*defaultdungheap);
	     manureInst->SetScalable(true);
        manureInst->ReadParameters(file);
   ...
   // Build the product stores
      ...
      if (pigs)
      {
			...
			strcpy(InstanceName,"PigHousing");
         GetSectionNumbers(InstanceName,&first,&num);
         Setcur_pos(0);
			defaultpigstable = new PigHousing(InstanceName,0,this);
         ...
         for(int InstanceIndex=first;InstanceIndex<(first+num);InstanceIndex++)
         {
				...
				PigHousing *stableInstance=new PigHousing(*defaultpigstable);
      ...
      if ((dairy)||(beef))
      {
			...
			strcpy(InstanceName,"Cattlehousing");
         GetSectionNumbers(InstanceName,&first,&num);
         Setcur_pos(0);
			defaultcattlehouse = new cattlehouse(InstanceName,0,this);
			defaultcattlehouse->ReadDefaultParameters();

         for(int InstanceIndex=first;InstanceIndex<(first+num);InstanceIndex++)
         {
//				cattlehouse *stableInstance=new cattlehouse(InstanceName,InstanceIndex,this,0);
				cattlehouse *stableInstance=new cattlehouse(*defaultcattlehouse);
            stableInstance->ReadParameters(file);
				//stableInstance->ReadDefaultParameters();
            theStables->InsertLast(stableInstance);
            if (theOutputData->DetailsData.getManureManageDetails()==true)
            	stableInstance->OpenDetailsFile();
         }
...
/*      InstanceName="HardStanding";
  ...
  */
   ...
   // Check to see if any animal house stores which should be linked to external storage are not.
	...
	{
		string error="Building:: error in opening "+InputFilename;
		theMessage->FatalError(error.c_str());
	}
