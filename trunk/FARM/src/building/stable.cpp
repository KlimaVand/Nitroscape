...
stable::stable(const stable& aStable):building(aStable)
{
	...
	for (int i=0;i<aStable.sections->NumOfNodes();i++)
	   {
		animalSection *p = new animalSection(*(aStable.sections->ElementAtNumber(i)));
	...
	for (int i=0;i<aStable.standartSections->NumOfNodes();i++)
	   {
		animalSection *p = new animalSection(*(aStable.standartSections->ElementAtNumber(i)));
...
void stable::ReadDefaultParameters(fstream * file)
{
	// name of instance must be set before running this function
	//	building::ReadParameters(file);  cost of animal housing is now the sum of the constituent parts (animal sections etc)
	Setfile(file);
