 ...
 */
void slurrystore::ReadParameters(fstream * afile)
{
	Setfile(afile);
	FindSection(aBase->GetName(), aBase->GetIndex()); // perform this in the top of hierarchy - and only there
	manurestore::ReadParameters(afile);
	building::ReadParameters(afile);
	Setfile(afile);
...
//   verbosity =true;
	   SetCritical();
	   FindSection("Slurrystore(0)");
	   building::ReadDefaultParameters();
		Setcur_pos(0);
	   FindSection("Slurrystore(0)");
	   manurestore::ReadDefaultParameters(file);
		Setcur_pos(0);
	   FindSection("Slurrystore(0)");
