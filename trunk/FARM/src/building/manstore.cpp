...
void manurestore::ReadParameters(fstream * afile)
{
	aBase->Setfile(afile);
	aBase->Setcur_pos(0);
	aBase->SetCritical();
	...
	aBase->UnsetCritical();
	aBase->GetParameter("ManureStoreID",&manureStoreID);
...
void manurestore::OpenDetailsFile()
{
	...
	hd.changeDir(theGlobalData->getOutputDirectory());
...
void manurestore::ReadDefaultParameters(fstream *afile)
{
	aBase->Setfile(afile);
	aBase->SetCritical();
   aBase->Setcur_pos(0);
   ...
   aBase->Setcur_pos(0);
	aBase->FindSection(aBase->GetName(),aBase->GetIndex());
	theStore->ReadDefaultParameters(afile);
	bool isLocked=true;
	//aBase->GetParameter("Locked",&isLocked);
	switch (theControlParameters->GetvolatModel())
	{
