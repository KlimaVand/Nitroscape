...
void cropParametersData::loaddata()
{
	...
	fileAccess hd;
	hd.changeDir(theGlobalData->getInputDirectory());
		...
		if(tmp[0]=='['&&tmp[tmp.size()-1]==']')
		{
			...
			for(unsigned int j=0;j<data.size();j++)
			{
				if(data.at(j).cropName.compare(name)==0)
				{
					if(primaryFile==true)
					{
						theMessage->FatalError("already read crop");
					}
					else
					{
						newData=data.at(j);
...
void cropParametersData::checkData()
{
    ...
    for(unsigned int i=0;i<beet.size();i++)
    {
    	...
    	if(tmp.MaxAllocToStorage<0)
    		theMessage->FatalError("cropParametersData:MaxAllocToStorage is not found in file");
    ...
    for(unsigned int i=0;i<ryeGrass.size();i++)
    {
    	...
    	if(tmp.BulkDensity<0)
    	    theMessage->FatalError("cropParametersData:BulkDensity is not found in file");
    	else if(tmp.InitialLAIIncrease<0)
    		theMessage->FatalError("cropParametersData:InitialLAIIncrease is not found in file");
    	else if(tmp.MinDayLength<0)
    	   theMessage->FatalError("cropParametersData:MinDayLength is not found in file");
    	else if(tmp.RateOfRipeReSeed<0)
    	   theMessage->FatalError("cropParametersData:RateOfRipeReSeed is not found in file");
    	else if(tmp.RecycleFraction<0)
    	    theMessage->FatalError("cropParametersData:RecycleFraction is not found in file");
    	else if(tmp.RespirationTop<0)
    	    theMessage->FatalError("cropParametersData:RespirationTop is not found in file");
    	else if(tmp.TransferRate<0)
    	    theMessage->FatalError("cropParametersData:TransferRate is not found in file");
    	else if(tmp.liveOMD<0)
    		theMessage->FatalError("cropParametersData:liveOMD is not found in file");
    	else if(tmp.deadOMD<0)
    	    theMessage->FatalError("cropParametersData:deadOMD is not found in file");
    }
    for(unsigned int i=0;i<clover.size();i++)
    {
    	cloverData tmp=clover.at(i);
    	if(tmp.NFixationThisDay<0)
    		theMessage->FatalError("cropParametersData:NFixationThisDay is not found in file");
    ...
    for(unsigned int i=0;i<rye.size();i++)
    {
    	...
    	if(tmp.HeightA<0)
    	  theMessage->FatalError("cropParametersData:HeightA is not found in file");
    	if(tmp.HeightB<0)
    	   theMessage->FatalError("cropParametersData:HeightB is not found in file");
     }
	for(unsigned int i=0;i<data.size();i++)
	{
		generalData newData=data.at(i);
		cout<<newData.cropName<<endl;
		if(newData.DMCostPerN<0)
			theMessage->FatalError("cropParametersData:DMCostPerN is not found in file");
		else if(newData.MaxFixPerDM<0)
			theMessage->FatalError("cropParametersData:MaxFixPerDM is not found in file");
		else if(newData.AmmoniumUptakeRate<0)
			theMessage->FatalError("cropParametersData:AmmoniumUptakeRate is not found in file");
		else if(newData.CoeffGreenIdx<0)
			theMessage->FatalError("cropParametersData:CoeffGreenIdx is not found in file");

		else if(newData.Conversfactor<0)
			theMessage->FatalError("cropParametersData:Conversfactor is not found in file");
		else if(newData.ConversionCoefficient<0)
			theMessage->FatalError("cropParametersData:ConversionCoefficient is not found in file");
		else if(newData.DecayRoot<0)
			theMessage->FatalError("cropParametersData:DecayRoot is not found in file");
		else if(newData.FillFactor<0)
			theMessage->FatalError("cropParametersData:FillFactor is not found in file");
		else if(newData.GerminationConversion<0)
			theMessage->FatalError("cropParametersData:GerminationConversion is not found in file");
		else if(newData.InitialCAI<0)
			theMessage->FatalError("cropParametersData:InitialCAI is not found in file");
		else if(newData.InitialRootDepth<0)
			theMessage->FatalError("cropParametersData:InitialRootDepth is not found in file");
		else if(newData.InterceptCoeff<0)
			theMessage->FatalError("cropParametersData:InterceptCoeff is not found in file");
		else if(newData.k<0)
			theMessage->FatalError("cropParametersData:k is not found in file");
		else if(newData.LAIDistributionPar<0)
			theMessage->FatalError("cropParametersData:LAIDistributionPar is not found in file");
		else if(newData.LAIDMRatio<0)
			theMessage->FatalError("cropParametersData:LAIDMRatio is not found in file");
		else if(newData.LAINitrogenRatio<0)
			theMessage->FatalError("cropParametersData:LAINitrogenRatio is not found in file");
		else if(newData.MaxGLAICrop<0)
			theMessage->FatalError("cropParametersData:MaxGLAICrop is not found in file");
		else if(newData.MaxAllocToRoot<0)
			theMessage->FatalError("cropParametersData:MaxAllocToRoot is not found in file");
		else if(newData.MaxDMtemp<0)
			theMessage->FatalError("cropParametersData:MaxDMtemp is not found in file");
		else if(newData.MaxPlantHeight<0)
			theMessage->FatalError("cropParametersData:MaxPlantHeight is not found in file");
		else if(newData.MaxRadUseEff<0)
			theMessage->FatalError("cropParametersData:MaxRadUseEff is not found in file");
		else if(newData.MinAllocToRoot<0)
			theMessage->FatalError("cropParametersData:MinAllocToRoot is not found in file");
		else if(newData.MinDMtemp<0)
		{
			...
			theMessage->FatalError("cropParametersData:MinimumRootDens is not found in file");
		else if(newData.MinimumSenescense<0)
			theMessage->FatalError("cropParametersData:MinimumSenescense is not found in file");
		else if(newData.MinimumSoilAmmonium<0)
			theMessage->FatalError("cropParametersData:MinimumSoilAmmonium is not found in file");
		else if(newData.MinimumSoilNitrate<0)
			theMessage->FatalError("cropParametersData:MinimumSoilNitrate is not found in file");
		else if(newData.MinN_Root<0)
			theMessage->FatalError("cropParametersData:MinN_Root is not found in file");
		else if(newData.MinN_Store<0)
			theMessage->FatalError("cropParametersData:MinN_Store is not found in file");
		else if(newData.NCurveParam>=0)
			theMessage->FatalError("cropParametersData:NCurveParam is not found in file");
		else if(newData.NitrateUptakeRate<0)
			theMessage->FatalError("cropParametersData:NitrateUptakeRate is not found in file");
		else if(newData.NPctMax<0)
			theMessage->FatalError("cropParametersData:NPctMax is not found in file");
		else if(newData.NPctMin<0)
			theMessage->FatalError("cropParametersData:NPctMin is not found in file");
		else if(newData.PhotoSyntActPart<0)
			theMessage->FatalError("cropParametersData:PhotoSyntActPart is not found in file");
		else if(newData.PowDM>=0)
			theMessage->FatalError("cropParametersData:PowDM is not found in file");
		else if(newData.ReducedNInYellow<0)
			theMessage->FatalError("cropParametersData:ReducedNInYellow is not found in file");
		else if(newData.RhizoDepositFraction<0)
			theMessage->FatalError("cropParametersData:RhizoDepositFraction is not found in file");
		else if(newData.RootDensAtBottom<0)
			theMessage->FatalError("cropParametersData:RootDensAtBottom is not found in file");
		else if(newData.RootPentrRate<0)
			theMessage->FatalError("cropParametersData:RootPentrRate is not found in file");
		else if(newData.RootpF<0)
			theMessage->FatalError("cropParametersData:RootpF is not found in file");
		else if(newData.SpecificRootLength<0)
			theMessage->FatalError("cropParametersData:SpecificRootLength is not found in file");
		else if(newData.StoreForFilling<0)
			theMessage->FatalError("cropParametersData:StoreForFilling is not found in file");
		else if(newData.TempRootMin<0)
			theMessage->FatalError("cropParametersData:TempRootMin is not found in file");
		else if(newData.TopFraction<0)
			theMessage->FatalError("cropParametersData:TopFraction is not found in file");
		else if(newData.WaterDeficitGrainFill<0)
			theMessage->FatalError("cropParametersData:WaterDeficitGrainFill is not found in file");
		else if(newData.WaterDeficitLagPhase<0)
			theMessage->FatalError("cropParametersData:WaterDeficitLagPhase is not found in file");
		else if(newData.WaterDeficitVegGrowth<0)
			theMessage->FatalError("cropParametersData:WaterDeficitVegGrowth is not found in file");
		else if(newData.FractionNminInRoots<0)
			theMessage->FatalError("cropParametersData:FractionNminInRoots is not found in file");
		else if(newData.FractionNminInVegTop<0)
			theMessage->FatalError("cropParametersData:FractionNminInVegTop is not found in file");
		else if(newData.CO2Effect<0)
			theMessage->FatalError("cropParametersData:CO2Effect is not found in file");
		else if(newData.MaxRootDepth<0)
			theMessage->FatalError("cropParametersData:MaxRootDepth is not found in file");
		else if(newData.RelativeDensity<0)
			theMessage->FatalError("cropParametersData:RelativeDensity is not found in file");
		else if(newData.MaxN_Store<0)
			theMessage->FatalError("cropParametersData:MaxN_Store is not found in file");
		else if(newData.DS_Flagligule<0)
			theMessage->FatalError("cropParametersData:DS_Flagligule is not found in file");
		else if(newData.DS_StartFill<0)
			theMessage->FatalError("cropParametersData:DS_StartFill is not found in file");
		else if(newData.TS0<0)
			theMessage->FatalError("cropParametersData:TS0 is not found in file");
		else if(newData.TS1<0)
			theMessage->FatalError("cropParametersData:TS1 is not found in file");
		else if(newData.TS2<0)
			theMessage->FatalError("cropParametersData:TS2 is not found in file");
		else if(newData.TS3<0)
			theMessage->FatalError("cropParametersData:TS3 is not found in file");
		else if(newData.TB0<0)
			theMessage->FatalError("cropParametersData:TB0 is not found in file");
		else if(newData.TB1<0)
			theMessage->FatalError("cropParametersData:TB1 is not found in file");
		else if(newData.TB2<0)
			theMessage->FatalError("cropParametersData:TB2 is not found in file");
		else if(newData.TB3<0)
			theMessage->FatalError("cropParametersData:TB3 is not found in file");
		else if(newData.DB<0)
			theMessage->FatalError("cropParametersData:DB is not found in file");
		else if(newData.DMAX<0)
			theMessage->FatalError("cropParametersData:DMAX is not found in file");
		else if(newData.LinearLeafPhase<0)
			theMessage->FatalError("cropParametersData:LinearLeafPhase is not found in file");
		else if(newData.MaxAllocToRoot<newData.MinAllocToRoot)
			theMessage->FatalError("cropParametersData:MaxAllocToRoot and MinAllocToRoot wrong");
		else if(newData.MaxDMtemp<newData.MinDMtemp)
			theMessage->FatalError("cropParametersData:MaxDMtemp and MinDMtemp wrong");
		else if(newData.MaxN_Root<newData.MinN_Root)
			theMessage->FatalError("cropParametersData:MaxN_Root and MinN_Root wrong");
		else if(newData.MaxN_Store<newData.MinN_Store)
		{
			cout<<newData.MaxN_Store<<endl;
			cout<<newData.MinN_Store<<endl;
			theMessage->FatalError("cropParametersData:MaxN_Store and MinN_Store wrong");
		}
