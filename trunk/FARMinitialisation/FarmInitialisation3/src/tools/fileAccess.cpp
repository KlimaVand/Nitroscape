 ...
 */
bool fileAccess::directoryExists(string path) {
	return directoryExists((char *) path.c_str());
}
 ...
 * @return	boolean	if the directory exists.
 */
bool fileAccess::directoryExists(const char * path) {
#ifndef VS2008
	struct stat st;
 ...
 */
bool fileAccess::fileExists(string path) {
	return fileExists((char *) path.c_str());
}
 ...
 * @return	boolean	if the file exists.
 */
bool fileAccess::fileExists(const char * path) {
	ifstream ifile(path);
	return ifile;
}
...
void fileAccess::openFile(char * filename,bool seachback)
{
	...
	if(!tmp)
	{
         ...
         cout << "In directory " << getCurrentPath() <<endl;
...
string fileAccess::getCurrentPath()
{
	...
	if (getcwd(path,sizeof(path)) == NULL) {
		cout << "error on fileAccess::getCurrentPath()" << endl;
	}
    CurrentPath=path;
    return CurrentPath;
}
...
int fileAccess::CreateDirectoryW(string directoryName)
{
	int retVal;
...
string fileAccess::getCheckedLineFile()
{
	string inputString = getLineFile();
	size_t aVal;
	aVal = inputString.find_last_of("\r");
	// if not found, returns npos, which is not always defined as -1
	if (aVal>0 && aVal != string::npos) {
		inputString.erase(aVal,1);
	}
	return inputString;
}
