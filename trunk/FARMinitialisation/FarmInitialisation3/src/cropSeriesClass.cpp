...
bool cropSeriesClass::getcrops(XMLNode *fieldNode, int startYear, int endYear)
{
	...
	for (int i=0;i<numNodes; i++)
	{
		...
		if (!xCropNode.isEmpty())
		{
			...
			{
				theMessage->WarningWithDisplay("Crop type ID missing in field", seriesNumber);
			...
			if (retVal)
			{
				...
				if (xNode.getText())
				{
					...
					if (doOp)
					{
						afieldOpSow->DefineOp ((char *) "Sowing", 2, year, month, day, *cropID);
				...
				if (xNode.getText())
				{
					...
					{
						afieldOpHarv->DefineOp ((char *) "Harvesting", 9, year, month, day, false);
			...
			if (xNode.getText())
			{
				...
				if (doOp)
				{
					...
					if (xNode.getText())
					{
						...
						if (tillageType=="3")
							tillDepth=250.0;
						afieldOpTill->DefineOp ((char *) "Tillage", 1, year, month, day, tillDepth);
				...
				if (doOp)
				{
					...
					if (xNode.getText())
					{
						...
						if (xNode.getText())
						{
							...
							afieldOpFert->DefineOp ((char *) "Fertilisation", 3, year, month, day, theType, ammonium, nitrate, organic,
									carbon,method);
				...
				if (doOp)
				{
					...
					if (xNode.getText())
					{
						...
						if (xNode.getText())
						{
							string fertMeth(xNode.getText());
							method= atoi(fertMeth.c_str());
							afieldOpFert->DefineOp ((char *) "Fertilisation", 3, year, month, day, theType, ammonium, nitrate, organicN,
									carbon,method);
							theOperations->InsertLast(afieldOpFert);
						}
						else
...
{
	string aVal;
	if (index<(int) theCrops.size())
		aVal.assign(theCrops.at(index).cropName);
	else
		aVal.assign("CROP MISSING");
	return aVal;
}
