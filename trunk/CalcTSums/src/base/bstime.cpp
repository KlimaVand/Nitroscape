...
{
	year = Year;
	CalcDateFromJulianDay(JulianDay);
}
/****************************************************************************\
\****************************************************************************/
...
/****************************************************************************\
\****************************************************************************/
void bsTime::SubtractOneDay(void)
{
   if(day > 1)
   {
      day -= 1;
   ...
   else
   {
	   ...
	   {
		   month--;
		   day = tabDaysPerMonth[IsLeapYear(year)][month-1];
		   dayInYear--;
	   }
   }
...
bool bsTime::CalcDateFromJulianDay(int aJulianday)
{
	bool gotit=false;
   int cumdayNo=0;
   int theDay=0;
   ...
   int isLeap = IsLeapYear(year);
	...
	while ((themonth<=12)&&(!gotit))
   {
      cumdayNo +=tabDaysPerMonth[isLeap][themonth];
      if (cumdayNo>=aJulianday)
      {
         gotit=true;
         theDay = tabDaysPerMonth[isLeap][themonth]-(cumdayNo - aJulianday);
      }
      themonth++;
   }
   day=theDay;
