...
/param radStem name of subfolder with radiation data
...
/param results[][366][4] array containing met data (air temp mean, air temp max, precipitation, radiation
 */
...
//    char Cclim_map1[200], Cclim_map2[200], Cclim_bin1[200], Cclim_bin2[200];
    string Cclim_map1, Cclim_map2, Cclim_bin1, Cclim_bin2;
    ...
    //Open files to write dayly/monthly/annual output
            ...
            else
            {
                ...
                for (int day=read_start[2];day<=read_end[2];day+=dim_stp[2])
                {
                    ...
                    for (int para=read_start[3];para<=read_end[3];para++)
                    {
                        ...
                        //the second one contining only radiation
...
int StringToInt(string conv) {
  int intReturn;
  intReturn = atoi(conv.c_str());
  return	intReturn;
}
struct marscoordinates {
  int first;
  int second;
} ;
static vector<marscoordinates> marsList;
static bool readbefore=false;
int getMARS(int hsmuID,char Cmars[256])
{

	if(readbefore==false)
	{
		files.open(Cmars,ios::in);
		if (files.fail()){theMessage->FatalError("getMetData: file ", Cmars, " not found");}
		while(!files.eof())
		{
			string input=getLineFile();
			stringstream totalSString( input );
			string colum1, colum2;
			totalSString >>colum1;
			totalSString >>colum2;
			marscoordinates tmp;
			tmp.first=StringToInt(colum1);
			tmp.second=StringToInt(colum2);
			marsList.push_back(tmp);
		}
		readbefore=true;
	}

	for (int i=0;i<marsList.size();i++)
	{
		if(marsList.at(i).first==hsmuID)
			return marsList.at(i).second;
	}

	theMessage->FatalError("getMetData: could not found Mars");
    return 0;
}
...
void calcTemperatureSums(int ID, int startYear, int endYear, char * inputDir,
	char * tpvStem, char * radStem, char * ncu_mars, int baseYear, int range, double baseTemp)
{
